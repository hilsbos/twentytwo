 ...
 *
 * This module statically imports all 16 hand-drawn SVG illustration components
 * (~238 KB of source). The APP only reaches it through `index.ts`'s dynamic
 * `import('./all')`, so the bundler keeps it in a SEPARATE async chunk that
 * loads on first form-sheet open — keeping the initial Today payload small.
 * Tests and the render-grid script import it directly (bundle size irrelevant).
 */
